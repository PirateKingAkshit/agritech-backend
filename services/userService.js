...
const createUser = async ({ phone, email, password, otp, first_name, last_name, location, state, city, address, role }) => {
  ...
  const generatedOtp = otp || Math.floor(100000 + Math.random() * 900000).toString(); // Generate OTP if not provided
  const otpExpires = new Date(Date.now() + 10 * 60 * 1000); // Expires in 10 minutes
  ...
  return { user: { id: user._id, phone, email, role: user.role }, otp: generatedOtp }; // Return OTP for testing
};
